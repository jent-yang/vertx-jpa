...
public class EntityManagerProviderImpl implements EntityManagerProvider {
	...
	@Override
	public EntityManagerProvider getEntityManager(Handler<AsyncResult<EntityManager>> resultHandler) {

		synchronized (vertx) {

			Future<EntityManager> future = Future.future();
			vertx.executeBlocking(f1 -> {
				...
				holder.entityManagerFactory().setHandler(res -> {
					if (res.succeeded()) {
						...
						future.complete(EntityManager.create(emf.createEntityManager()));
					} else {
	...
	private class EmfProvidereHolder implements Shareable {
		...
		public EmfProvidereHolder(VertxInternal vertx, JsonObject config, LocalMap<String, EmfProvidereHolder> map,
				String name) {
			this.name = name;
			this.config = config;
			this.vertx = vertx;
