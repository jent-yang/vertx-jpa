	...
	public Future<EntityManagerFactory> getEntityManagerFactory(String datasourceName) {
		...
		synchronized (vertx) {
			...
			vertx.executeBlocking(f1 -> {
				...
				config.getJsonArray("annotated_classes", new JsonArray()).stream().forEach(p -> scanAnnotatedClasses(p.toString(), persistenceUnitInfo));
...
//		ClassAnnotationMatchProcessor processor = spui::addAnnotatedClassName; TODO
	...
	private Properties createProperties() {
		...
		properties.setProperty("hibernate.dialect", config.getString("dialect", HibernateDialectProvider.dialectFromUrl(config.getString("url", "jdbc:h2:mem:test_mem;"))));
